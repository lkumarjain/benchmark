...
func BenchmarkStdLib(b *testing.B) {
	...
	for _, tt := range tests {
		benchmarkStdLibStruct(b, tt.name, []byte(tt.payload), tt.size)
		benchmarkStdLibInterface(b, tt.name, []byte(tt.payload), tt.size)
	}
}

func benchmarkStdLibStruct(b *testing.B, prefix string, payload []byte, size int) {
...
func benchmarkStdLibInterface(b *testing.B, prefix string, payload []byte, size int) {
