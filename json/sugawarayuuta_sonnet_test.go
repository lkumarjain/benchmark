...
import (
	...
	encoding "github.com/sugawarayuuta/sonnet"
)

func BenchmarkSonnet(b *testing.B) {
	b.ReportAllocs()
	for _, tt := range tests {
		benchmarkSonnetStruct(b, tt.name, []byte(tt.payload), tt.size)
		benchmarkSonnetInterface(b, tt.name, []byte(tt.payload), tt.size)
	}
}

func benchmarkSonnetStruct(b *testing.B, prefix string, payload []byte, size int) {
...
func benchmarkSonnetInterface(b *testing.B, prefix string, payload []byte, size int) {
